...
class Actor_RNN(nn.Module):
    def __init__(self, args, actor_input_dim):
        ...
        self.fc1 = nn.Linear(actor_input_dim, args.rnn_hidden_dim).to(device)
        self.rnn = nn.GRUCell(args.rnn_hidden_dim, args.rnn_hidden_dim).to(device)
        self.fc2 = nn.Linear(args.rnn_hidden_dim, args.action_dim).to(device)
...
class Critic_RNN(nn.Module):
    def __init__(self, args, critic_input_dim):
        ...
        self.fc1 = nn.Linear(critic_input_dim, args.rnn_hidden_dim).to(device)
        self.rnn = nn.GRUCell(args.rnn_hidden_dim, args.rnn_hidden_dim).to(device)
        self.fc2 = nn.Linear(args.rnn_hidden_dim, 1).to(device)
        self.activate_func = [nn.Tanh(), nn.ReLU()][args.use_relu]
        if args.use_orthogonal_init:
            print("------use_orthogonal_init------")
            orthogonal_init(self.fc1)
...
class Actor_MLP(nn.Module):
    def __init__(self, args, actor_input_dim):
        ...
        self.fc1 = nn.Linear(actor_input_dim, args.mlp_hidden_dim).to(device)
        self.fc2 = nn.Linear(args.mlp_hidden_dim, args.mlp_hidden_dim).to(device)
        self.fc3 = nn.Linear(args.mlp_hidden_dim, args.action_dim).to(device)
        self.activate_func = [nn.Tanh(), nn.ReLU()][args.use_relu]
...
class Critic_MLP(nn.Module):
    def __init__(self, args, critic_input_dim):
        ...
        self.fc1 = nn.Linear(critic_input_dim, args.mlp_hidden_dim).to(device)
        self.fc2 = nn.Linear(args.mlp_hidden_dim, args.mlp_hidden_dim).to(device)
        self.fc3 = nn.Linear(args.mlp_hidden_dim, 1).to(device)
        self.activate_func = [nn.Tanh(), nn.ReLU()][args.use_relu]
        if args.use_orthogonal_init:
            print("------use_orthogonal_init------")
            orthogonal_init(self.fc1)
    ...
    def train(self, replay_buffer, total_steps):
        ...
        for _ in range(self.K_epochs):
            for index in BatchSampler(SequentialSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                if (batch['a_n'][index] > 11).any():
                    print("____________")
                    print(batch["a_n"][index])
                    a_n_now = torch.clamp(batch['a_n'][index], 0, 11)
                    a_logprob_n_now = dist_now.log_prob(a_n_now)
                else:
